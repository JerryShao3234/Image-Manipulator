...
printf( "(no spaces and no more than %d chars including .bmp extension):\n",
   FILE_NAME_SIZE - 1 );
scanf( "%s", inFileName );
inFileName[ FILE_NAME_SIZE - EXT_LENGTH - 1 ] = '\0';  /* in case user 
entered too many characters */
if( readFile( inFileName, header, &red, &green, &blue, &imgWidth, 
&imgHeight ) )
    {
...
if( userOption == FILTER )
        ...
        scanf( "%s", outFileName );
        outFileName[ FILE_NAME_SIZE - EXT_LENGTH - 1 ] = '\0';  /* in case user 
entered too many characters */
        addExtension( outFileName );
        if( !writeFile( outFileName, header, red, green, blue, imgSize, 
imgWidth ) )
 ...
 * Param: red - address of array that will contain red component of image pixels at
end of function call
 * Param: green - address of array that will contain green component of image 
pixels at end of function call
 * Param: blue - address of array that will contain blue component of image pixels 
 ...
 * Return: TRUE if uncompressed bmp image of size WIDTH by HEIGHT is read from 
file, FALSE otherwise
 */
int readFile( char inFileName[], char header[], 
 int** red, int** green, int** blue, int* imgWidth, int* 
imgHeight )
{
    FILE* inFile;
    int bytesRead;
 ...
 * Param: header - will contain file header information at end of function call
 * Param: inFile - pointer to file containing image (assumed to be open for reading
in binary mode)
 ...
 * Param: red - array that will contain red component of image pixels (stored row 
by row) at end of function call
 * Param: green - array that will contain green component of image pixels (stored 
row by row) at end of function call
 * Param: blue - array that will contain blue component of image pixels (stored row
by row) at end of function call
 * Param: imgSize - size of image in pixels
 ...
 */
int readRawData( int* red, int* green, int* blue, int imgSize, int imgWidth, FILE* 
inFile )
{
    int index;
    int bytesRead = 0;
    ...
    {
        bytesRead += fread( buffer, sizeof( char ), BIT_DEPTH / BITS_PER_BYTE, 
inFile );
 ...
 * Param: red - array that contains red component of image pixels (stored row by 
row)
 * Param: green - array that contains green component of image pixels (stored row 
by row)
 * Param: blue - array that contains blue component of image pixels (stored row by 
row)
 * Param: imgSize - size of image in pixels
 * Param: imgWidth - width of image in pixels
 * Return: TRUE if image was written to file, FALSE otherwise
 */
int writeFile( char fileName[], char header[], int* red, int* green, int* blue, int
imgSize, int imgWidth )
{
    FILE* outFile;
    int bytesWritten;
    int rawOffset;
    ...
    if( outFile != NULL )
    {
        ...
        bytesWritten = writeRawData( red, green, blue, imgSize, imgWidth, 
outFile );
 ...
 * Param: header - image header information
 * Param: outFile - pointer to file to which image is to be written (assumed to be 
 ...
 * Return: number of bytes written to file
 */
int writeRawData( int* red, int* green, int* blue, int imgSize, int imgWidth, FILE*
outFile )
{
    int index;
    int bytesWritten = 0;
    ...
    for( index = 0; index < imgSize; index++ )
    {
        ...
        bytesWritten += fwrite( buffer, sizeof( char ), BIT_DEPTH / BITS_PER_BYTE, 
outFile );
        /* check for end of row */
        if( index % imgWidth == imgWidth - 1 )
        {
